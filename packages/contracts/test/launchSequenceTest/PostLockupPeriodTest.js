...
contract('After the initial lockup period has passed', async accounts => {
    ...
    // changed dec 24 to dec 22 to match smaller LQTY mint, and dec 23 to dec 21

    // 1e24 = 1 million tokens with 18 decimal digits
    const teamMemberInitialEntitlement_1 = dec(1, 22)
    const teamMemberInitialEntitlement_2 = dec(2, 22)
    const teamMemberInitialEntitlement_3 = dec(3, 22)

    const investorInitialEntitlement_1 = dec(4, 22)
    const investorInitialEntitlement_2 = dec(5, 22)
    const investorInitialEntitlement_3 = dec(6, 22)

    const teamMemberMonthlyVesting_1 = dec(1, 21)
    const teamMemberMonthlyVesting_2 = dec(2, 21)
    const teamMemberMonthlyVesting_3 = dec(3, 21)

    const LQTYEntitlement_A = dec(1, 22)
    const LQTYEntitlement_B = dec(2, 22)
    const LQTYEntitlement_C = dec(3, 22)
    const LQTYEntitlement_D = dec(4, 22)
    const LQTYEntitlement_E = dec(5, 22)
    ...
    describe('Beneficiary withdrawal from initial LC', async accounts => {
        ...
        it("Beneficiaries can withraw full LQTY balance of LC if it has increased since lockup period ended", async() => {
            ...
            await lqtyToken.transfer(LC_I1.address, dec(1, 22), { from: multisig })
            await lqtyToken.transfer(LC_I2.address, dec(1, 22), { from: multisig })
            await lqtyToken.transfer(LC_I3.address, dec(1, 22), { from: multisig })
    ...
    describe('Transferring LQTY', async accounts => {
        it("LQTY multisig can transfer LQTY to LCs they deployed", async() => {
            ...
            await lqtyToken.transfer(LC_T1.address, dec(1, 22), { from: multisig })
            await lqtyToken.transfer(LC_T2.address, dec(1, 22), { from: multisig })
            await lqtyToken.transfer(LC_T3.address, dec(1, 22), { from: multisig })
            ...
            assert.isTrue(LQTYBalanceOfLC_T1_1.eq(th.toBN(initialLQTYBalanceOfLC_T1).add(th.toBN(dec(1, 22)))))
            assert.isTrue(LQTYBalanceOfLC_T2_1.eq(th.toBN(initialLQTYBalanceOfLC_T2).add(th.toBN(dec(1, 22)))))
            assert.isTrue(LQTYBalanceOfLC_T3_1.eq(th.toBN(initialLQTYBalanceOfLC_T3).add(th.toBN(dec(1, 22)))))
            ...
            assert.isTrue(LQTYBalanceOfLC_T1_2.eq(LQTYBalanceOfLC_T1_1.add(th.toBN(dec(1, 22)))))
            assert.isTrue(LQTYBalanceOfLC_T2_2.eq(LQTYBalanceOfLC_T2_1.add(th.toBN(dec(1, 22)))))
            assert.isTrue(LQTYBalanceOfLC_T3_2.eq(LQTYBalanceOfLC_T3_1.add(th.toBN(dec(1, 22)))))
        })
        ...
        it("LQTY multisig can transfer tokens to LCs deployed by anyone", async() => {
            ...
            await lqtyToken.transfer(LC_A.address, dec(1, 22), { from: multisig })
            await lqtyToken.transfer(LC_B.address, dec(2, 22), { from: multisig })
            await lqtyToken.transfer(LC_C.address, dec(3, 22), { from: multisig })

            // Check balances of LCs have increased
            assert.equal(await lqtyToken.balanceOf(LC_A.address), dec(1, 22))
            assert.equal(await lqtyToken.balanceOf(LC_B.address), dec(2, 22))
            assert.equal(await lqtyToken.balanceOf(LC_C.address), dec(3, 22))
        })
        ...
        it("LQTY multisig can transfer LQTY directly to any externally owned account", async() => {
            ...
            const txA = await lqtyToken.transfer(A, dec(1, 22), { from: multisig })
            const txB = await lqtyToken.transfer(B, dec(2, 22), { from: multisig })
            const txC = await lqtyToken.transfer(C, dec(3, 22), { from: multisig })

            // Check new balances have increased by correct amount
            assert.equal(await lqtyToken.balanceOf(A), dec(1, 22))
            assert.equal(await lqtyToken.balanceOf(B), dec(2, 22))
            assert.equal(await lqtyToken.balanceOf(C), dec(3, 22))
        })

        it("Anyone can transfer LQTY to LCs deployed by anyone", async() => {
            // Start D, E, F with some LQTY
            await lqtyToken.transfer(D, dec(1, 22), { from: multisig })
            await lqtyToken.transfer(E, dec(2, 22), { from: multisig })
            await lqtyToken.transfer(F, dec(3, 22), { from: multisig })
            ...
            await lqtyToken.transfer(LCAddress_A, dec(1, 22), { from: D })
            await lqtyToken.transfer(LCAddress_B, dec(2, 22), { from: E })
            await lqtyToken.transfer(LCAddress_C, dec(3, 22), { from: F })

            // Check balances of LCs has increased
            assert.equal(await lqtyToken.balanceOf(LCAddress_A), dec(1, 22))
            assert.equal(await lqtyToken.balanceOf(LCAddress_B), dec(2, 22))
            assert.equal(await lqtyToken.balanceOf(LCAddress_C), dec(3, 22))
        })


        it("Anyone can transfer to an EOA", async() => {
            // Start D, E, liquityAG with some LQTY
            await lqtyToken.unprotectedMint(D, dec(1, 22))
            await lqtyToken.unprotectedMint(E, dec(2, 22))
            await lqtyToken.unprotectedMint(liquityAG, dec(3, 22))
            await lqtyToken.unprotectedMint(multisig, dec(4, 22))
