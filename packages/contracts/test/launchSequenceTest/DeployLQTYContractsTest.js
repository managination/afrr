...
contract('Deploying the LQTY contracts: LCF, CI, LQTYStaking, and LQTYToken ', async accounts => {
    ...
    const expectedCISupplyCap = oneHundred.mul(oneMillion).mul(digits)
    ...
    describe('LQTYToken deployment', async accounts => {
        ...
        it("Mints the correct LQTY amount to the multisig's address: (1 million)", async() => {
            const multisigLQTYEntitlement = await lqtyToken.balanceOf(multisig)
                // 1 million as 18-digit decimal
            const _1Million = dec(1, 24)
            assert.equal(multisigLQTYEntitlement, _1Million)
        })

        it("Mints the correct LQTY amount to the CommunityIssuance contract address: 100 million", async() => {
            const communityLQTYEntitlement = await lqtyToken.balanceOf(communityIssuance.address)
                // 100 million as 18-digit decimal
            const _100Million = dec(100, 24)

            assert.equal(communityLQTYEntitlement, _100Million)
        })
    ...
    describe('Community Issuance deployment', async accounts => {
        ...
        it("Has a supply cap of 100 million", async() => {
            const supplyCap = await communityIssuance.LQTYSupplyCap()

            assert.isTrue(expectedCISupplyCap.eq(supplyCap))
        })

        it("Liquity AG can set addresses if CI's LQTY balance is equal or greater than 100 million ", async() => {
            ...
            const tx = await communityIssuance.setAddresses(
                lqtyToken.address,
                coreContracts.stabilityPool.address, { from: liquityAG }
            );
            assert.isTrue(tx.receipt.status)
        })

        it("Liquity AG can't set addresses if CI's LQTY balance is < 100 million ", async() => {
            ...
            await lqtyToken.transfer(newCI.address, '999999999999999999999999', { from: multisig })
