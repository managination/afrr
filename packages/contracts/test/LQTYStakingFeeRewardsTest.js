...
contract('LQTYStaking revenue share tests', async accounts => {
  ...
  describe.only('stake and unstake', function () {
    ...
    it.only("LQTY Staking: A single staker earns all ETH and LQTY fees that occur", async () => {
      ...
      const A_LUSDDebt_Before = toBN(await troveManager.getTroveDebt(A))

      // A un-stakes
      await lqtyStaking.unstake(dec(100, 18), {from: A, gasPrice: 0})

      const A_ETHBalance_After = toBN(await web3.eth.getBalance(A))
      const A_LUSDDebt_After = toBN(await troveManager.getTroveDebt(A))


      const A_ETHGain = A_ETHBalance_After.sub(A_ETHBalance_Before)
      const A_LUSDGain = A_LUSDDebt_Before.sub(A_LUSDDebt_After)
    ...
    it.only("LQTY Staking: Multiple stakers earn the correct share of all ETH and LQTY fees, based on their stake size", async () => {
      ...
      */
      ...
      const A_LUSDBalance_Before = toBN(await troveManager.getTroveDebt(A))
      const B_LUSDBalance_Before = toBN(await troveManager.getTroveDebt(B))
      const C_LUSDBalance_Before = toBN(await troveManager.getTroveDebt(C))
      const D_LUSDBalance_Before = toBN(await troveManager.getTroveDebt(D))
      ...
      const A_LUSDBalance_After = toBN(await troveManager.getTroveDebt(A))
      const B_LUSDBalance_After = toBN(await troveManager.getTroveDebt(B))
      const C_LUSDBalance_After = toBN(await troveManager.getTroveDebt(C))
      const D_LUSDBalance_After = toBN(await troveManager.getTroveDebt(D))
      ...
      const A_LUSDGain = A_LUSDBalance_Before.sub(A_LUSDBalance_After)
      const B_LUSDGain = B_LUSDBalance_Before.sub(B_LUSDBalance_After)
      const C_LUSDGain = C_LUSDBalance_Before.sub(C_LUSDBalance_After)
      const D_LUSDGain = D_LUSDBalance_Before.sub(D_LUSDBalance_After)
